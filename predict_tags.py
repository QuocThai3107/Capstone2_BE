...
class TagPredictor:
    def __init__(self, model_path, vectorizer_path, tags_path):
        ...
        print(f"Đường dẫn đầy đủ: {os.path.abspath(vectorizer_path)}")
        print(f"File tồn tại: {os.path.exists(vectorizer_path)}")
        try:
            self.vectorizer = joblib.load(vectorizer_path)
            print("Đã tải thành công vectorizer")
            print(f"Type of vectorizer: {type(self.vectorizer)}")
        except Exception as e:
            print(f"Lỗi khi tải vectorizer: {str(e)}")
            traceback.print_exc()
            raise
        ...
        try:
            ...
            print("Đã tải thành công tags")
        except Exception as e:
            print(f"Lỗi khi tải tags: {str(e)}")
            traceback.print_exc()
            raise
    ...
    def predict_tags(self, text, top_k=5):
        ...
        try:
            ...
            print(f"Kích thước vector sau điều chỉnh: {text_vector_dense.shape}")
            ...
            print(f"Lỗi trong predict_tags: {str(e)}")
            traceback.print_exc()
            raise
...
def main():
    ...
    print(f"Model tồn tại: {os.path.exists(model_path)}")
    print(f"Vectorizer tồn tại: {os.path.exists(vectorizer_path)}")
    print(f"Tags tồn tại: {os.path.exists(tags_path)}")
